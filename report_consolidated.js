...
$.getJSON('manifest.webapp').done(manifest => {
	...
	selection.setListenerFunction(function(e){
		...
		$.getJSON("../../../api/organisationUnits/"+ouId+".json", function(ouDetail) {
			...
			$("#visualization").load('https://cdn.jsdelivr.net/gh/padamdahal/HMIS-App@master/report-parts-consolidated.html');
	...
	function getReport(ouid, reportType){
		var dx;
		$.getJSON("https://cdn.jsdelivr.net/gh/padamdahal/HMIS-App@master/config_consolidated.json", function(configs) {
			//$("#visualization").empty();
			$(".card-body").empty();
